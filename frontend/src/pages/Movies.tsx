...
const MOVIES_FILTERS_KEY = 'smartcinema_movies_filters';
...
// Certification options for movies
const CERTIFICATION_OPTIONS = [
    { value: 'G', label: 'G (عام)' },
    { value: 'PG', label: 'PG (إرشاد أبوي)' },
    { value: 'PG-13', label: 'PG-13 (13+)' },
    { value: 'R', label: 'R (مقيد 17+)' },
...
function loadFilters(): FiltersState {
    try {
        const saved = sessionStorage.getItem(MOVIES_FILTERS_KEY);
        if (saved) {
...
function saveFilters(filters: FiltersState) {
    try {
        sessionStorage.setItem(MOVIES_FILTERS_KEY, JSON.stringify(filters));
    } catch {
...
const Movies = () => {
    ...
    const [inputValue, setInputValue] = useState(urlSearch || initialFilters.inputValue);
    ...
    }, [search, inputValue, selectedGenres, selectedCountries, selectedCertifications, selectedQualities, yearRange, sortBy, showFilters]);
    ...
    // Fetch ALL movies first (no pre-filtering)
    const { data: allMovies = [], isLoading } = useQuery({
        queryKey: ['media', 'movies', sortBy],
        queryFn: () =>
    ...
    const allCountries = Array.from(new Set(allMovies.map((m: Media) => m.country).filter(Boolean))) as string[];

    // Get all unique qualities
    const allQualities = Array.from(new Set(allMovies.map((m: Media) => m.quality).filter(Boolean))) as string[];
    // Sort qualities (prioritize known ones)
    ...
    const allCertifications = Array.from(new Set(allMovies.map((m: Media) => m.certification).filter(Boolean))) as string[];
    ...
    const filteredMovies = allMovies.filter((movie) => {
        ...
        if (selectedCountries.length > 0) {
            if (!selectedCountries.includes(movie.country || '')) return false;
        }

        // Quality filter (multi-select)
        if (selectedQualities.length > 0) {
            const movieQuality = movie.quality || 'Unknown';
            if (!selectedQualities.includes(movieQuality)) return false;
        }

        // Year filter
        if (movie.year) {
            if (movie.year < yearRange[0] || movie.year > yearRange[1]) return false;
        }

        // Certification filter (multi-select)
        if (selectedCertifications.length > 0) {
            if (!selectedCertifications.includes(movie.certification || '')) return false;
        }
        ...
        <div className="min-h-screen bg-background">
            ...
            <main className="container mx-auto px-4 py-8 mt-20">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-4xl font-bold mb-2">الأفلام</h1>
                    ...
                    >
                        ...
                        {search && (
                            <Badge variant="secondary" className="gap-1">
                                بحث: {search}
                                <X className="w-3 h-3 cursor-pointer" onClick={() => { setSearch(''); setInputValue(''); }} />
                            </Badge>
                        )}
                                ...
                                <div className="flex-1">
                                    ...
                                    <Select value={sortBy} onValueChange={setSortBy}>
                                        ...
                                        <SelectContent>
                                            ...
                                            <SelectItem value="tmdb_rating">الأعلى تقييماً</SelectItem>
